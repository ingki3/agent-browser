...
- **정정 (2026-09-23)**: 이때 추가된 신호 중 '포커스 이동'은 효과가 아니라 도달의 증거였다. Mock 골든 버튼 대부분이 눌러도 아무 일도 하지 않았는데 포커스 이동만으로 성공 판정됐다(클릭 성공 판정의 55~60%). 규칙 6은 지켰지만 **신호를 추가해 게이트를 회복할 때는 그 신호가 무엇의 증거인지 사보타주로 확인해야 한다** — 무반응 버튼을 두고 신호가 성공을 내면 그 신호는 틀린 것이다. 교정 내역: `.hermes/plans/2026-09-23_postcondition-effect-signals.md`
...
#### 세션 CLI — 비밀번호를 저장하지 않는 로그인 (WS-21)

`agent-browser session login`은 **비밀번호를 저장하지 않습니다.** 사람이
실제 브라우저에서 로그인하고, 결과물인 쿠키/localStorage만 암호화해
보관합니다. PRD 2절 Persona A가 규정한 정규 경로입니다.

```
session login    headed 브라우저 -> 사람이 로그인 -> 암호화 저장
session list     프로파일 목록 (패스프레이즈 불필요, 내용은 못 봄)
session check    복호화 + 실제 접속으로 유효성 판정
session remove   삭제
```

**저장 전에 로그인 여부를 확인합니다.** 이 검사가 없으면 로그인 실패
상태의 빈 세션이 저장되고, 그 사실을 무인 실행 중에야 알게 됩니다.
PRD 5.1의 세션 프로브를 그대로 씁니다.

| 신호 | 판정 |
| :--- | :--- |
| 보이는 `input[type=password]` 개수 | 있으면 미로그인 의심 |
| URL이 `/login`·`/auth` 등을 포함 | 리다이렉트된 것으로 판정 |
| HTTP 401/403 | 만료 |

가시성 검사가 중요합니다. 많은 사이트가 로그인 후에도 **숨은 로그인
폼을 DOM에 남깁니다.** `display:none`이나 크기 0인 요소를 세면
로그인된 세션을 만료로 오판합니다.

**요약 출력에 값을 넣지 않습니다.** `list`는 프로파일 이름·크기·권한만
보여주고 복호화하지 않습니다. `check`는 복호화하지만 도메인 이름만
출력하고 쿠키 값은 절대 찍지 않습니다.

##### 실사용 검증에서 찾은 결함 (WS-21)

mock과 example.com으로만 검증하고 끝냈다면 못 찾았을 것들입니다.
실제 사이트와 파이프 환경에서 돌려보고 2건을 찾았습니다.

**1. 무인 실행이 막혔습니다.** `AGENT_AUTH_KEY_CI`를 설정해뒀는데도
프롬프트가 먼저 떴습니다. PRD 5.1-1 우선순위상 프롬프트가 2순위이고
환경변수가 3순위라 정상 동작인데, **CLI가 대화형 여부를 판단하지
않은 것**이 문제였습니다.

**2. 파이프 환경에서 스택트레이스가 노출됐습니다.** `getpass`가
`EOFError`를 던지고 그대로 터졌습니다. 확인 프롬프트 4곳이 모두
같은 문제를 가지고 있었습니다.

`_is_interactive()`로 tty를 확인하고, `_confirm()`으로 확인 프롬프트를
통일했습니다. **파괴적 동작(remove)의 비대화형 기본값은 False**입니다.

##### 사보타주가 테스트의 미탐을 두 번 잡았습니다

1차에 `_is_interactive`를 무력화해도 통과했습니다. 원인은 테스트가
그 함수를 **monkeypatch로 대체**해서, 함수 자체가 망가져도 드러나지
않은 것입니다. 가짜 스트림으로 실제 tty 판정을 검증하는 테스트를
추가해 탐지하게 했습니다.

> **교훈**: monkeypatch로 대체한 함수는 그 테스트가 검증하지 않는다.
> 대체 대상 자체를 검증하는 테스트가 따로 필요하다.

##### 소셜 로그인은 이 방법뿐입니다

구글·카카오 등은 헤드리스 자동화를 능동 탐지해 차단합니다. 플레이스홀더
치환(WS-20)으로도 안 됩니다. 사람이 직접 로그인하는 수밖에 없고,
그래서 `session login`이 그 유일한 경로입니다.

#### 자격증명 플레이스홀더 치환 (WS-20)

WS-19의 마스킹은 **사후 방어**였습니다. 값이 이미 LLM 프롬프트로
전송된 뒤 기록할 때만 가립니다. 이 기능은 전송 자체를 없앱니다.
...
Playwright MCP의 `--secrets`와 같은 방식입니다. PRD 5.3에 규정을
추가했습니다(사람 승인 후 개정).

**설계 결정 3가지**

1. **키 형식을 좁게 잡는다** — `^[A-Z][A-Z0-9_-]{2,63}$`. 일반 텍스트가
   키로 오인되면 사용자 입력이 조용히 바뀐다.
2. **미등록 키는 원본 그대로 전달한다** — 빈 값으로 바꾸면 사용자는
   왜 로그인이 실패하는지 알 수 없다. `secret_resolved: False`로 보고한다.
3. **원본 params를 변형하지 않는다** — 사본을 만들어 치환한다. 원본을
   바꾸면 호출자의 트레이스에 평문이 섞인다.

3번은 사보타주로 발견했습니다. 처음 테스트는 `dict(params)` 사본을
넘겨서 오염이 드러나지 않았고, **원본 변형 사보타주를 통과시켰습니다.**
사본 없이 넘기도록 고쳐서 탐지하게 했습니다.

**한계**: 치환된 값은 DOM에 존재하므로 이후 관찰·스크린샷에 노출될 수
있습니다. "LLM 컨텍스트 유입 차단"만 보장합니다.

#### 비밀번호 마스킹 규정 미준수 (WS-19)

PRD 5.3(525행)이 규정한 4대 마스킹 대상 중 **2건이 미준수**였습니다.
실측으로 확인했습니다.

```
[누출] 비밀번호 인풋 필드   {"text": "P@ssw0rd..."} -> 그대로
[ OK ] Authorization 헤더
[누출] Set-Cookie 헤더      session=sk-abc...7890 -> 원본 토큰 잔존
[ OK ] URL 쿼리스트링 토큰
```

**원인 1 — 키 이름 의존.** 마스킹기는 `password`/`secret` 같은 키
이름으로 민감 필드를 판정하는데, 액션 파라미터의 키는 `text`라는
중립적 이름입니다. 값도 평범한 문자열이라 정규식으로 잡히지 않습니다.

접근성 role로도 구분되지 않습니다. **실측 — `input[type=password]`와
일반 텍스트 입력이 모두 `role=textbox`입니다.** DOM을 직접 아는
디스패처만이 판정할 수 있어 `is_sensitive_field()`를 추가했습니다.

**원인 2 — 구조체 미적용.** 헤더 규칙은 `set-cookie: v` 형태의
문자열만 매칭합니다. `{"Set-Cookie": "session=..."}`처럼 키/값으로
분리되면 걸리지 않았습니다. `_SENSITIVE_KEY`에 추가했습니다.

##### 마스킹을 보안 경계로 여기지 말 것

외부 조사에서 확인한 사실입니다. Playwright MCP도 같은 문제를 겪었고
(issue #922), 메인테이너가 직접 이렇게 답했습니다.

> "가능하지만 자격증명은 DOM 스냅샷, 네트워크 페이로드 등으로 샐 수
> 있습니다. MCP를 devtools를 연 사용자라고 생각하십시오. 숨길 수는
> 있지만 안전하다고 부를 수는 없습니다."

실제로 응답 마스킹을 넣고 **1년이 지나서야** 콘솔 로그 유출 경로를
막았고(0.0.77), 트레이스와 HAR은 아직 미해결입니다.

> **원칙**: 마스킹은 방어의 한 겹입니다. 로그인이 필요한 작업은
> 세션 저장 방식(사람이 1회 로그인 -> `storage_state` 암호화 -> 재사용)을
> 쓰는 것이 근본적입니다. PRD 2절 Persona A가 규정한 방식이기도 합니다.

##### 소셜 로그인은 지원 대상이 아니다

구글·페이스북 로그인 페이지 직접 조작은 **의도적 미지원**입니다.
업계 공통 결론이며, 조사에서 확인한 근거는 다음과 같습니다.

| 서비스 | 대응 |
| :--- | :--- |
| Playwright 공식 문서 | `storageState` 재사용 권장 |
| Browserbase | 2FA는 사람에게 넘김(Session Live View) |
| Steel / Browser Use | "Human takeover for authentication" |

차단 사유는 헤드리스 지문, WebDriver 플래그, CAPTCHA, 계정 잠금,
수시로 바뀌는 UI입니다. 실측에서도 구글 검색이 `/sorry/index`로,
쿠팡이 403으로 막혔습니다.
