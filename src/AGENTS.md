...
| 지표 | 값 |
| :--- | :--- |
| 태스크 완수율 (독립 검증) | **23/25 = 92.0%** |
| 난이도별 | easy 4/4 · medium 5/5 · **hard 5/5** · multistep 5/6 · dynamic 4/5 |
| 태스크당 비용 (중앙값) | $0.0003 |
| 태스크당 스텝 (중앙값) | 3 |
| 25개 전체 비용 | $0.0223 |

WS-13에서 hard 티어가 **2/3 → 5/5**가 됐습니다(신규 2종 포함). 오래 실패하던
`hn-comments`도 통과했습니다. 전체 비율이 94.4%에서 92.0%로 내려간 것은
난이도가 높은 태스크 7종이 추가됐기 때문입니다.

실패 2건은 **모두 LLM 토큰 소진**이며 런타임 결함이 아닙니다. 재실행 시
같은 태스크가 성공하므로(2/2) 모델 판단의 플레이키성으로 분류합니다.
...
#### 기각된 대응: max_tokens 재시도 (WS-13)

reasoning 계열 모델이 특정 스텝에서 사고 토큰을 소진해 본문을 못 내놓는
현상이 있습니다. 상한을 3배로 늘려 재시도하는 방식을 시도했으나 **기각**했습니다.
...
여유를 주면 모델이 더 긴 사고를 이어갈 뿐, 결론에 도달하지 못했습니다.
토큰 소진은 예산 문제가 아니라 **모델 특성**이므로 상한 조정이 아니라
모델 교체로 대응해야 합니다.
