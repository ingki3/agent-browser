...
| 지표 | 값 |
| :--- | :--- |
| 태스크 완수율 (독립 검증) | **17/18 = 94.4%** |
| 난이도별 | easy 4/4 · medium 5/5 · hard 2/3 · multistep 6/6 |
| 태스크당 비용 (중앙값) | $0.0004 |
| 태스크당 스텝 (중앙값) | 3 |
| 태스크당 소요 (중앙값) | 28.5초 |
| 18개 전체 비용 | $0.0100 |

난이도는 4단계입니다. `multistep`은 5스텝 이상, 이전 스텝의 결과 위에
다음 액션을 쌓아야 완료되는 태스크입니다(TodoMVC 항목 3개 추가 등).
2~4스텝 태스크만으로는 긴 호흡의 실패 모드를 관측할 수 없습니다.
