...
##### melon-chart: 루프는 이를 구분할 수 없다

첫 클릭으로 이미 목표를 달성했는데, LLM이 확인차 같은 요소를 한 번
더 클릭해 실패했고, 그 실패 때문에 완료 선언이 막혔습니다.

가드를 "성공한 액션이 하나라도 있으면 통과"로 완화하려 했으나,
**두 사례의 트레이스 모양이 동일**해 구분이 불가능했습니다.

```
hn-comments (막아야 함)      melon-chart (통과시켜야 함)
  click @e1 -> OK              click @e2 -> OK
  click @e2 -> FAIL            click @e2 -> FAIL
  finish                       finish
```

차이는 액션 성공 여부가 아니라 **목표 달성 여부**입니다. 그건 목표를
아는 독립 검증만이 판정할 수 있습니다. 루프는 목표 달성을 알 방법이
없습니다.

> **결론**: 가드는 현행 유지합니다. `silent_win`(성공을 놓침)이
> `false_claim`(거짓 성공)보다 안전한 실패 방향이고, 독립 검증이
> 최종 판정을 하므로 완수율 지표는 영향받지 않습니다.
>
> 대신 이 한계를 문서화합니다 — **에이전트의 자기 보고는 보수적으로
> 편향돼 있습니다.** 실제로 성공한 것을 실패로 보고할 수 있습니다.
